...
OBS_CONFIG = {
    ...
    1: {"name": "Leading Muon eta", "bins": np.linspace(-2.5, 2.5, 40)},
    2: {"name": "ZZ Mass (m_zz) [GeV]", "bins": np.linspace(100, 1000, 40)},
    3: {"name": "Delta R between Zs", "bins": np.linspace(0, 6, 40)}
}

OUTPUT_DIR = "/content/drive/MyDrive/Unfolding_pp>zz_DNN-Results_with loss_cort_E"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Global Scaler for Neural Network Normalization
global_scaler = StandardScaler()


# 2. PHYSICS & RECONSTRUCTION

def calculate_dilepton_mass(pt1, eta1, phi1, pt2, eta2, phi2):
    """Calculate invariant mass of two leptons (muon mass assumption)."""
    MUON_MASS = 0.10566
...
def calculate_fourlepton_mass(pts, etas, phis):
    """Calculate invariant mass of four leptons."""
    MUON_MASS = 0.10566
    px_sum = py_sum = pz_sum = e_sum = 0

    for pt, eta, phi in zip(pts, etas, phis):
        px_sum += pt * np.cos(phi)
        py_sum += pt * np.sin(phi)
        pz_sum += pt * np.sinh(eta)
        e_sum += np.sqrt(px_sum**2 + py_sum**2 + pz_sum**2 + MUON_MASS**2)

    m2 = e_sum**2 - px_sum**2 - py_sum**2 - pz_sum**2
    return np.sqrt(m2) if m2 > 0 else 0

def calculate_composite_eta_phi(pt1, eta1, phi1, pt2, eta2, phi2):
    """
    Calculate pseudorapidity (eta) and phi of a composite particle.
    FIX: Uses geometric definition of Eta (-ln(tan(theta/2))).
    """
    ...
    if p > 0 and abs(pz) < p:
        ...
        theta = np.clip(theta, 1e-6, np.pi - 1e-6) # Numerical stability
...
def reconstruct_zz_event(pts, etas, phis, charges):
    """
    Reconstruct ZZ from 4 muons.
    """
    ...
    # 1. Find leading muon (always the hardest one)
    leading_idx = np.argmax(pts)
    leading_pt = pts[leading_idx]
    leading_eta = etas[leading_idx]

    # 2. Find best Z pairing
    Z_MASS = 91.1876
    best_score = float('inf')
    best_result = None

    # SEARCH STRATEGY: Look at top N muons to save time
    search_limit = min(n_muons, 6)

    # Loop 1: First Z candidate (indices i, j)
    for i in range(search_limit):
        for j in range(i+1, search_limit):

            # Check Charge: Must be opposite
            if charges[i] * charges[j] >= 0: continue

            # Loop 2: Second Z candidate (indices k, l) from REMAINING muons
            others = [x for x in range(search_limit) if x != i and x != j]

            # We need at least 2 remaining muons
            if len(others) < 2: continue

            # Check pairs within the remaining set
            for idx_k in range(len(others)):
                for idx_l in range(idx_k + 1, len(others)):
                    k = others[idx_k]
                    l = others[idx_l]

                    if charges[k] * charges[l] >= 0: continue

                    # Calculate Z masses
                    mz1 = calculate_dilepton_mass(pts[i], etas[i], phis[i], pts[j], etas[j], phis[j])
                    mz2 = calculate_dilepton_mass(pts[k], etas[k], phis[k], pts[l], etas[l], phis[l])

                    if mz1 <= 0 or mz2 <= 0: continue

                    # Score: Minimize total mass difference from Z pole
                    score = abs(mz1 - Z_MASS) + abs(mz2 - Z_MASS)

                    if score < best_score:
                        best_score = score

                        # Calculate Event Properties using these 4 best muons
                        # Note: We pass the specific 4 indices to mass calc if we wanted,
                        # but simple 4-mass of the event usually takes the 4 constituents.
                        # Here we recalculate using the specific 4 chosen muons:

                        chosen_pts = np.array([pts[i], pts[j], pts[k], pts[l]])
                        chosen_etas = np.array([etas[i], etas[j], etas[k], etas[l]])
                        chosen_phis = np.array([phis[i], phis[j], phis[k], phis[l]])

                        zz_mass = calculate_fourlepton_mass(chosen_pts, chosen_etas, chosen_phis)

                        # Delta R between Zs
                        eta1_z, phi1_z = calculate_composite_eta_phi(
                            pts[i], etas[i], phis[i], pts[j], etas[j], phis[j]
                        )
                        eta2_z, phi2_z = calculate_composite_eta_phi(
                            pts[k], etas[k], phis[k], pts[l], etas[l], phis[l]
                        )
                        delta_r = calculate_delta_r(eta1_z, phi1_z, eta2_z, phi2_z)

                        best_result = (leading_pt, leading_eta, zz_mass, delta_r)

    return best_result

def load_data(filename, is_data=False):
    """
    FIXED LOADING:
    - Loads Reco from 'Muon' branch.
    - Loads Gen from 'Particle' branch (only for MC).
    - Checks efficiency correctly.
    """
    ...
    try:
        ...
        # RECONSTRUCTED LEVEL (Muon Branch)
        reco_mu_pt = tree["Muon.PT"].array()
        reco_mu_eta = tree["Muon.Eta"].array()
        reco_mu_phi = tree["Muon.Phi"].array()
        reco_mu_charge = tree["Muon.Charge"].array()
        n_events = len(reco_mu_pt)

        reco_feats = np.zeros((n_events, 4), dtype=np.float32)
        reco_success = np.zeros(n_events, dtype=bool)

        # GENERATOR LEVEL (Particle Branch)
        if not is_data:
            gen_mu_pt = tree["Particle.PT"].array()
            gen_mu_eta = tree["Particle.Eta"].array()
            gen_mu_phi = tree["Particle.Phi"].array()
            gen_mu_charge = tree["Particle.Charge"].array()
            gen_mu_pid = tree["Particle.PID"].array()

            gen_feats = np.zeros((n_events, 4), dtype=np.float32)
            gen_success = np.zeros(n_events, dtype=bool)

        print(f"Processing {n_events} events...")

        for i in range(n_events):
            # --- Reco ---
            r_pts = ak.to_numpy(reco_mu_pt[i])
            if len(r_pts) >= 4:
                r_res = reconstruct_zz_event(
                    r_pts, ak.to_numpy(reco_mu_eta[i]),
                    ak.to_numpy(reco_mu_phi[i]), ak.to_numpy(reco_mu_charge[i])
                )
                if r_res:
                    reco_feats[i] = r_res
                    reco_success[i] = True
            ...
            # --- Gen (MC Only) ---
            if not is_data:
                # Filter for Muons (PID +/- 13)
                pids = ak.to_numpy(gen_mu_pid[i])
                is_mu = np.abs(pids) == 13

                if np.sum(is_mu) >= 4:
                    g_res = reconstruct_zz_event(
                        ak.to_numpy(gen_mu_pt[i])[is_mu], ak.to_numpy(gen_mu_eta[i])[is_mu],
                        ak.to_numpy(gen_mu_phi[i])[is_mu], ak.to_numpy(gen_mu_charge[i])[is_mu]
                    )
                    if g_res:
                        gen_feats[i] = g_res
                        gen_success[i] = True
        ...
        # APPLY CUTS 
        # Fiducial cuts: pT > 10, Mass > 0
        reco_pass = reco_success & (reco_feats[:,0] > 10) & (reco_feats[:,2] > 0)

        if not is_data:
            gen_pass = gen_success & (gen_feats[:,0] > 10) & (gen_feats[:,2] > 0)

            # Intersection mask: Valid events must exist in BOTH Reco and Gen
            combined_mask = reco_pass & gen_pass

            X_reco = reco_feats[combined_mask]
            X_gen = gen_feats[combined_mask]

            print(f"MC Loaded. Raw: {n_events}, Passing (Reco+Gen): {len(X_reco)}")
            return X_reco, X_gen
        else:
            X_reco = reco_feats[reco_pass]
            print(f"Data Loaded. Raw: {n_events}, Passing (Reco): {len(X_reco)}")
            return X_reco
...
MC_PATH = "/content/drive/MyDrive/Thesis_Files/delphes_pp4uu.root"
DATA_PATH = "/content/drive/MyDrive/Thesis_Files/delphes_pp4uu_high.root"

# Check if data needs to be loaded or reloaded
if 'X_mc_reco' not in locals() or \
   'X_mc_gen' not in locals() or \
   'X_data_reco' not in locals():
    print("Loading Data...")
    res = load_data(MC_PATH, is_data=False)
    if res is not None:
    ...
    print("Data (X_mc_reco, X_mc_gen, X_data_reco) already present in memory.")
...
def prepare_tensor(data, fit_scaler=False):
    ...
    # Only columns to log transform: 0=LeadingMuPt, 2=ZZMass
    # Column 1=eta (can be negative), 3=DeltaR (already well-behaved)
    cols = [0, 2]
    ...
    # Log transform only selected columns
...
def run_fast_ensemble(mc_reco, mc_gen, data_reco, n_runs=3, n_iters=2):
    """
    Original ensemble function with NaN fixes - ADAPTED FOR ZZ
    """
    print(f"Starting Fast Ensemble ({n_runs} runs)...")
    start_time = time.time()

    # 1. Fix negative values BEFORE fitting scaler
    # UPDATE: Only columns that need log transform for ZZ process
    cols = [0, 2]  # LeadingMuPt, ZZMass
...
def plot_and_save_results(mc_gen, w_mean, w_err, title, filename_tag, injection_lines=None):
    ...
    for i, idx in enumerate(OBS_CONFIG.keys()):
        ...
        # Add units based on observable
            ...
            #xlabel_text += " [GeV]"
        if idx == 1:  # Leading Muon eta (dimensionless)
            xlabel_text += ""
        #elif idx == 2:  # ZZ Mass
            #xlabel_text += " [GeV]"
        elif idx == 3:  # Delta R between Zs (dimensionless)
            xlabel_text += ""
    ...
    # Save
    save_path = os.path.join(OUTPUT_DIR, f"{filename_tag}.png")
...
def save_csv_data(w_mean, w_err, stats, filename_tag):
    ...
    df_weights.to_csv(os.path.join(OUTPUT_DIR, f"{filename_tag}_weights.csv"),
                     index_label="EventID")
    pd.DataFrame(stats).to_csv(os.path.join(OUTPUT_DIR, f"{filename_tag}_stats.csv"),
                              index=False)
...
# Run Unfolding
w_m_std, w_e_std = run_fast_ensemble(X_mc_reco, X_mc_gen, X_data_reco, n_runs=5, n_iters=4)
...
# UPDATE: Use ZZ observable names
observables = ['Leading Muon pT [GeV]', 'Leading Muon eta', 'ZZ Mass [GeV]', 'Delta R between Zs']
...
plt.tight_layout()
weights_diag_path = os.path.join(OUTPUT_DIR, "Phase1_Weights_Diagnostics.png")
...
plt.tight_layout()
closure_path = os.path.join(OUTPUT_DIR, "Phase1_Closure_Test.png")
...
plt.tight_layout()
ensemble_path = os.path.join(OUTPUT_DIR, "Phase1_Ensemble_Stability.png")
...
plt.tight_layout()
pulls_path = os.path.join(OUTPUT_DIR, "Phase1_Pull_Distribution.png")
...
weight_stats_path = os.path.join(OUTPUT_DIR, "Phase1_Weight_Statistics.json")
import json
...
print(f"5. Weight statistics: {weight_stats_path}")
...
try:
    ...
    # Create a new directory if OUTPUT_DIR doesn't exist
    save_dir = "/content/drive/MyDrive/Unfolding_pp4uu_Results"
...
# C. LOAD OR ACCESS YOUR DATA

print(f"\n{'='*80}")
print("DATA CHECK")
print(f"{'='*80}")

# Access your data based on the variables defined in your notebook
try:
    # These should exist after running your execution block
    mc_gen_data = X_mc_gen
    mc_reco_data = X_mc_reco
    weights = w_m_std  # Unfolded weights

    print("Using X_mc_gen, X_mc_reco, and w_mean variables")

except NameError as e:
    print(f"ERROR: Required data variables not found! {e}")
    print("\nTrying to load data directly...")

    # If variables don't exist, try to load the data
    try:
        # Use your load_data function
        MC_PATH = "/content/drive/MyDrive/Thesis_Files/PP4UU/delphes_pp4uu.root"
        DATA_PATH = "/content/drive/MyDrive/Thesis_Files/PP4UU/delphes_pp4uu_high.root"

        print("Loading data from files...")
        res = load_data(MC_PATH, is_data=False)
        if res is not None:
            mc_reco_data, mc_gen_data = res
            print("Loaded MC data successfully")

        # For weights, we need to run unfolding first
        print("WARNING: No weights found. Running unfolding to get weights...")
        # You would need to run your unfolding code here
...
# E. DEFINE FEATURE LABELS BASED ON YOUR OBS_CONFIG

# Based on our OBS_CONFIG dictionary with 4 observables
feature_labels = [
    "Leading muon pT [GeV]",          # OBS_CONFIG[0]
    "Leading muon eta",                 # OBS_CONFIG[1]
    "ZZ Mass [GeV]",               # OBS_CONFIG[2]
    "delta_r(Z1, Z2)"                   # OBS_CONFIG[3]
]

# F. PLOT 1: THE CORRELATION HEATMAPS
...
plt.suptitle("Correlation Matrix Analysis: ZZ-> muonmuonmuonmuon\nPreserving Multi-Dimensional Structure",
             fontsize=16, fontweight='bold', y=1.05)
plt.tight_layout()

# Save the heatmap plot
heatmap_path = os.path.join(corr_dir, "correlation_heatmaps.png")
plt.savefig(heatmap_path, dpi=300, bbox_inches='tight', facecolor='white')
print(f"Saved heatmap plot: {heatmap_path}")

# Also save as PDF
heatmap_pdf_path = os.path.join(corr_dir, "correlation_heatmaps.pdf")
plt.savefig(heatmap_pdf_path, bbox_inches='tight', facecolor='white')
print(f"Saved heatmap plot (PDF): {heatmap_pdf_path}")

plt.show()

# G. PLOT 2: 2D SCATTER / DENSITY COMPARISON (ALL PAIRS)

print(f"\nGenerating 2D correlation plots for all variable pairs...")

# Define all interesting pairs for ZZ->muonmuonmuonmuon
correlation_pairs = [
    (0, 2, "Leading muon pT [GeV]", "ZZ Mass [GeV]", "pT-Mass Correlation"),
    (0, 1, "Leading muon pT [GeV]", "Leading muon eta", "pT-eta Correlation"),
    (2, 3, "ZZ Mass [GeV]", "delta_r(Z1, Z2)", "Mass-Angular Correlation"),
    (1, 2, "Leading muon eta", "ZZ Mass [GeV]", "eta-Mass Correlation"),
    (0, 3, "Leading muon pT [GeV]", "delta_r(Z1, Z2)", "pT-Angular Correlation"),
    (1, 3, "Leading muon eta", "delta_r(Z1, Z2)", "eta-Angular Correlation")
]
...
summary_path = os.path.join(corr_dir, "2d_correlations_summary.png")
plt.savefig(summary_path, dpi=300, bbox_inches='tight', facecolor='white')
print(f"Saved 2D correlations summary: {summary_path}")
plt.show()

# H. PLOT 3: INDIVIDUAL 2D PLOTS
...
for idx_x, idx_y, lbl_x, lbl_y, title in correlation_pairs[:3]:  # First 3 most important
    ...
    individual_path = os.path.join(corr_dir, f"2d_{safe_title}.png")
    plt.savefig(individual_path, dpi=300, bbox_inches='tight', facecolor='white')
    print(f"Saved: {individual_path}")

    plt.show()

# I. PLOT 4: CORRELATION COEFFICIENT COMPARISON (BAR PLOT)
...
plt.suptitle('Quantitative Correlation Analysis: ZZ->muonmuonmuonmuon',
             fontsize=16, fontweight='bold', y=1.05)
plt.tight_layout()

# Save bar plot
barplot_path = os.path.join(corr_dir, "correlation_barplot.png")
plt.savefig(barplot_path, dpi=300, bbox_inches='tight', facecolor='white')
print(f"Saved correlation bar plot: {barplot_path}")
plt.show()


# J. PLOT 5: CORRELATION DIFFERENCE HEATMAP
...
diff_path = os.path.join(corr_dir, "correlation_differences_detailed.png")
plt.savefig(diff_path, dpi=300, bbox_inches='tight', facecolor='white')
print(f"Saved detailed difference plot: {diff_path}")
plt.show()

# K. CREATE SUMMARY TEXT FILE

print(f"\nCreating summary report...")

summary_path = os.path.join(corr_dir, "analysis_summary.txt")
with open(summary_path, 'w') as f:
    ...
    f.write(f"Dataset: ZZ->muonmuonmuonmuon (4-lepton final state)\n")
...
# L. PRINT FINAL SUMMARY OF CORRELATION ANALYSIS
...
def run_1d_ibu_zz_benchmark(rec_train, gen_train, data_obs, bins, n_iterations=4):
    """
    Performs standard 1D Iterative Bayesian Unfolding for ZZ process.
    """
...
# Define which observables to compare (using your OBS_CONFIG for ZZ)
# UPDATE: Choose key ZZ observables to compare
observables_to_compare = {
    0: {"name": "Leading Muon pT [GeV]", "bins": np.linspace(0, 300, 40)},
    1: {"name": "Leading Muon eta", "bins": np.linspace(-2.5, 2.5, 40)},
    2: {"name": "ZZ Mass (m_zz) [GeV]", "bins": np.linspace(100, 1000, 40)},
    3: {"name": "Delta R between Zs", "bins": np.linspace(0, 5, 40)}
}
...
for idx, config in observables_to_compare.items():
    ...
    # A. DNN UNFOLDING RESULTS
    print("\n1. DNN Unfolding (Our method):")
    data = X_mc_gen[:, idx]
    ...
    x_ibu, y_ibu_counts, err_ibu_counts = run_1d_ibu_zz_benchmark(
    ...
    plt.tight_layout()
    comparison_path = os.path.join(OUTPUT_DIR, f"Phase1_Comparison_DNN_vs_IBU_{var_name.replace(' ', '_').replace('(', '').replace(')', '')}.png")
...
# A. Inject signal directly into the DATA file (X_data_reco)
print("Injecting synthetic signal bumps into data...")

# Use the existing data
X_data_with_signal = X_data_reco.copy()

# Make the signal STRONGER and more focused
ZZ_MASS_BUMP = 500.0    # Signal ZZ mass bump (GeV)
DR_ZZ_BUMP = 2.0        # Signal Delta R between Zs bump
LEADING_MUON_PT_BUMP = 180.0  # Increased to make more visible

# Inject into MORE events with STRONGER signal
n_events = len(X_data_with_signal)
n_inject = int(n_events * 0.50)  # INCREASED to 50% of events
indices_to_inject = np.random.choice(n_events, n_inject, replace=False)

print(f"Injecting STRONG signal into {n_inject} events ({100*n_inject/n_events:.1f}% of data)")

# For a CLEARER signal, make correlated injections
# Real ZZ signals would affect all observables together, not independently
print("  -> Creating correlated ZZ signal events (affecting all observables)")
for idx in indices_to_inject:
    # Create a correlated signal
    signal_strength = np.random.uniform(1.0, 1.3)  # How strong this signal event is

    # 1. Increase ZZ mass (make it a resonance)
    current_mass = X_data_with_signal[idx, 2]
    X_data_with_signal[idx, 2] = ZZ_MASS_BUMP + np.random.normal(0, 25)  # Resonance at 500 GeV

    # 2. Increase leading muon pT (correlated with mass)
    X_data_with_signal[idx, 0] *= signal_strength * 1.2  # 20-56% increase

    # 3. Set specific Delta R (intermediate separation)
    X_data_with_signal[idx, 3] = DR_ZZ_BUMP + np.random.normal(0, 0.3)  # Peak at 2.0

    # 4. Adjust eta slightly (real signal might have different kinematics)
    X_data_with_signal[idx, 1] += np.random.normal(0, 0.1)

print(f"Data with signal shape: {X_data_with_signal.shape}")

# B. Run unfolding
print("\nRunning unfolding on signal-injected data...")
w_m_sig, w_e_sig = run_fast_ensemble(X_mc_reco, X_mc_gen, X_data_with_signal, n_runs=6, n_iters=6)

# C. Plot with IMPROVED visualization
print("\nPlotting results with signal injection lines...")

# Define where we injected signals
injection_lines = {
    0: LEADING_MUON_PT_BUMP,  # Leading muon pT
    2: ZZ_MASS_BUMP,          # ZZ mass
    3: DR_ZZ_BUMP             # Delta R between Zs
}
...
stats_sig = plot_and_save_results(
    ...
    w_m_sig,
    w_e_sig,
    f"Phase 2: Signal Injection Test\nZZ Mass @{ZZ_MASS_BUMP}GeV, delta_r(Zs) @{DR_ZZ_BUMP}, Leading muon pT @{LEADING_MUON_PT_BUMP}GeV",
    "Phase2_Signal_Injection",
    injection_lines=injection_lines
)

# Save results
save_csv_data(w_m_sig, w_e_sig, stats_sig, "Phase2_Signal_Injection")
...
DR_ZZ_BUMP = 2.0
DR_ZZ_WIDTH = 0.4
LEADING_MUON_PT_BUMP = 150.0
LEADING_MUON_PT_WIDTH = 15.0

# Get the data from the unfolding plots
pt_idx = 0      # Leading Muon pT
mass_idx = 2    # ZZ Mass
dr_idx = 3      # Delta R between Zs

# Get bin edges from config
pt_bins = OBS_CONFIG[pt_idx]['bins']
...
pt_signal_min = LEADING_MUON_PT_BUMP - 1.5 * LEADING_MUON_PT_WIDTH
pt_signal_max = LEADING_MUON_PT_BUMP + 1.5 * LEADING_MUON_PT_WIDTH
mass_signal_min = ZZ_MASS_BUMP - 1.5 * ZZ_MASS_WIDTH
mass_signal_max = ZZ_MASS_BUMP + 1.5 * ZZ_MASS_WIDTH
dr_signal_min = DR_ZZ_BUMP - 1.5 * DR_ZZ_WIDTH
dr_signal_max = DR_ZZ_BUMP + 1.5 * DR_ZZ_WIDTH

# Define sideband regions (background control)
pt_sideband_min = LEADING_MUON_PT_BUMP - 4 * LEADING_MUON_PT_WIDTH
pt_sideband_max = LEADING_MUON_PT_BUMP - 2 * LEADING_MUON_PT_WIDTH
mass_sideband_min = ZZ_MASS_BUMP - 4 * ZZ_MASS_WIDTH
mass_sideband_max = ZZ_MASS_BUMP - 2 * ZZ_MASS_WIDTH
dr_sideband_min = DR_ZZ_BUMP - 4 * DR_ZZ_WIDTH
dr_sideband_max = DR_ZZ_BUMP - 2 * DR_ZZ_WIDTH

print(f"Leading Muon pT signal region: {pt_signal_min:.1f} - {pt_signal_max:.1f} GeV")
print(f"Leading Muon pT sideband region: {pt_sideband_min:.1f} - {pt_sideband_max:.1f} GeV")
print(f"ZZ Mass signal region: {mass_signal_min:.1f} - {mass_signal_max:.1f} GeV")
print(f"ZZ Mass sideband region: {mass_sideband_min:.1f} - {mass_sideband_max:.1f} GeV")
print(f"Delta R(Zs) signal region: {dr_signal_min:.3f} - {dr_signal_max:.3f}")
print(f"Delta R(Zs) sideband region: {dr_sideband_min:.3f} - {dr_sideband_max:.3f}")

# 2. Calculate histograms for unfolded data
print("\n2. Calculating histograms...")

# Unfolded distribution (with weights)
h_unfold_pt, _ = np.histogram(X_mc_gen[:, pt_idx], bins=pt_bins, weights=w_m_sig[:len(X_mc_gen)])
...
h_prior_pt, _ = np.histogram(X_mc_gen[:, pt_idx], bins=pt_bins, density=False)
...
# pT bins
pt_signal_bins = (pt_centers >= pt_signal_min) & (pt_centers <= pt_signal_max)
pt_sideband_bins = (pt_centers >= pt_sideband_min) & (pt_centers <= pt_sideband_max)
...
# Leading Muon pT
N_obs_pt = np.sum(h_unfold_pt[pt_signal_bins])
N_bkg_pt = np.sum(h_prior_pt_scaled[pt_signal_bins])
N_sig_pt = N_obs_pt - N_bkg_pt
...
print(f"Leading Muon pT:")
print(f"  -> Observed in signal region: {N_obs_pt:.1f}")
print(f"  -> Expected background: {N_bkg_pt:.1f}")
print(f"  -> Signal excess: {N_sig_pt:.1f}")
...
sig_pt = calculate_significance(N_obs_pt, N_bkg_pt)
sig_mass = calculate_significance(N_obs_mass, N_bkg_mass)
sig_dr = calculate_significance(N_obs_dr, N_bkg_dr)

print(f"Leading Muon pT significance: {sig_pt:.2f}sigma")
print(f"ZZ Mass significance: {sig_mass:.2f}sigma")
print(f"Delta R between Zs significance: {sig_dr:.2f}sigma")
...
pt_signal_area = np.sum(pt_signal_bins)
pt_sideband_area = np.sum(pt_sideband_bins)
...
bkg_pt_est, bkg_pt_err = estimate_background_with_error(
    h_unfold_pt[pt_signal_bins],
    h_unfold_pt[pt_sideband_bins],
    pt_signal_area,
    pt_sideband_area
)
...
print(f"\nLeading Muon pT (sideband method):")
print(f"  -> Expected background: {bkg_pt_est:.1f} +/- {bkg_pt_err:.1f}")
print(f"  -> Signal excess: {N_obs_pt - bkg_pt_est:.1f}")

print(f"\nZZ Mass (sideband method):")
print(f"  -> Expected background: {bkg_mass_est:.1f} +/- {bkg_mass_err:.1f}")
print(f"  -> Signal excess: {N_obs_mass - bkg_mass_est:.1f}")

print(f"\nDelta R between Zs (sideband method):")
...
sig_pt_robust = calculate_significance_with_error(N_obs_pt, bkg_pt_est, bkg_pt_err)
...
print(f"  -> Leading Muon pT: {sig_pt_robust:.2f}sigma")
print(f"  -> ZZ Mass: {sig_mass_robust:.2f}sigma")
print(f"  -> Delta R between Zs: {sig_dr_robust:.2f}sigma")

# 7. Visualize significance regions
print("\n7. Creating significance visualization plot...")

fig_sig, axes_sig = plt.subplots(1, 3, figsize=(18, 5))  # 3 panels for 3 observables

# Plot 1: Leading Muon pT distribution
ax1 = axes_sig[0]
ax1.step(pt_centers, h_unfold_pt, where='mid', label='Unfolded',
         color='black', linewidth=2, linestyle='--')
ax1.step(pt_centers, h_prior_pt_scaled, where='mid', label='SM Prior',
         color='blue', linewidth=1.5, alpha=0.8)

# Highlight signal region
ax1.axvspan(pt_signal_min, pt_signal_max, alpha=0.3, color='red',
           label=f'Signal region ({sig_pt:.1f}sigma)')
# Highlight sideband region
ax1.axvspan(pt_sideband_min, pt_sideband_max, alpha=0.2, color='green',
           label='Sideband (bkg est.)')
ax1.axvline(LEADING_MUON_PT_BUMP, color='red', linestyle='-', alpha=0.7, linewidth=1)

ax1.set_xlabel('Leading Muon pT [GeV]')
ax1.set_ylabel('Events')
ax1.set_title(f'Leading muon pT: Sig = {sig_pt:.1f}sigma ({sig_pt_robust:.1f}sigma robust)')
ax1.legend(fontsize=9)
ax1.grid(True, alpha=0.3)

# Add text box with results
text_pt = f"""Observed: {N_obs_pt:.1f}
Expected: {N_bkg_pt:.1f}
Excess: {N_sig_pt:.1f}
Simple: {sig_pt:.1f}sigma
Robust: {sig_pt_robust:.1f}sigma"""
ax1.text(0.05, 0.95, text_pt, transform=ax1.transAxes, fontsize=9,
        verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))

# Plot 2: ZZ Mass distribution
ax2 = axes_sig[1]
ax2.step(mass_centers, h_unfold_mass, where='mid', label='Unfolded',
         color='black', linewidth=2, linestyle='--')
ax2.step(mass_centers, h_prior_mass_scaled, where='mid', label='SM Prior',
         color='blue', linewidth=1.5, alpha=0.8)

# Highlight signal region
ax2.axvspan(mass_signal_min, mass_signal_max, alpha=0.3, color='red',
           label=f'Signal region ({sig_mass:.1f}sigma)')
# Highlight sideband region
ax2.axvspan(mass_sideband_min, mass_sideband_max, alpha=0.2, color='green',
           label='Sideband (bkg est.)')
ax2.axvline(ZZ_MASS_BUMP, color='red', linestyle='-', alpha=0.7, linewidth=1)

ax2.set_xlabel('ZZ Mass [GeV]')
ax2.set_ylabel('Events')
ax2.set_title(f'ZZ Mass: Sig = {sig_mass:.1f}sigma ({sig_mass_robust:.1f}sigma robust)')
...
Simple: {sig_mass:.1f}sigma
Robust: {sig_mass_robust:.1f}sigma"""
ax2.text(0.05, 0.95, text_mass, transform=ax2.transAxes, fontsize=9,
        verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))

# Plot 3: Delta R between Zs distribution
ax3 = axes_sig[2]
ax3.step(dr_centers, h_unfold_dr, where='mid', label='Unfolded',
         color='black', linewidth=2, linestyle='--')
ax3.step(dr_centers, h_prior_dr_scaled, where='mid', label='SM Prior',
         color='blue', linewidth=1.5, alpha=0.8)

# Highlight signal region
ax3.axvspan(dr_signal_min, dr_signal_max, alpha=0.3, color='red',
           label=f'Signal region ({sig_dr:.1f}sigma)')
# Highlight sideband region
ax3.axvspan(dr_sideband_min, dr_sideband_max, alpha=0.2, color='green',
           label='Sideband (bkg est.)')
ax3.axvline(DR_ZZ_BUMP, color='red', linestyle='-', alpha=0.7, linewidth=1)

ax3.set_xlabel('Delta R between Zs')
ax3.set_ylabel('Events')
ax3.set_title(f'delta_r(Zs): Sig = {sig_dr:.1f}sigma ({sig_dr_robust:.1f}sigma robust)')
...
Excess: {N_sig_dr:.1f}
Simple: {sig_dr:.1f}sigma
Robust: {sig_dr_robust:.1f}sigma"""
ax3.text(0.05, 0.95, text_dr, transform=ax3.transAxes, fontsize=9,
        verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8))

plt.tight_layout()
sig_plot_path = os.path.join(OUTPUT_DIR, "Phase2_Signal_Significance.png")
...
summary_data = {
    'Observable': ['Leading Muon pT', 'ZZ Mass (m_zz)', 'Delta R between Zs'],
    'Injection Point': [f'{LEADING_MUON_PT_BUMP} GeV', f'{ZZ_MASS_BUMP} GeV', f'{DR_ZZ_BUMP}'],
    'Observed Events': [f'{N_obs_pt:.1f}', f'{N_obs_mass:.1f}', f'{N_obs_dr:.1f}'],
    'Expected Background': [f'{N_bkg_pt:.1f}', f'{N_bkg_mass:.1f}', f'{N_bkg_dr:.1f}'],
    'Signal Excess': [f'{N_sig_pt:.1f}', f'{N_sig_mass:.1f}', f'{N_sig_dr:.1f}'],
    'Simple Significance': [f'{sig_pt:.2f}sigma', f'{sig_mass:.2f}sigma', f'{sig_dr:.2f}sigma'],
    'Robust Significance': [f'{sig_pt_robust:.2f}sigma', f'{sig_mass_robust:.2f}sigma', f'{sig_dr_robust:.2f}sigma']
}
...
summary_path = os.path.join(OUTPUT_DIR, "Phase2_Signal_Significance_Summary.csv")
df_summary.to_csv(summary_path, index=False)
print(f"\nSummary saved to: {summary_path}")

# Additional: Combined significance (if signals are independent)
print("\n" + "-" * 95)
print("COMBINED SIGNIFICANCE (assuming independent observables):")
combined_sig_simple = np.sqrt(sig_pt**2 + sig_mass**2 + sig_dr**2)
combined_sig_robust = np.sqrt(sig_pt_robust**2 + sig_mass_robust**2 + sig_dr_robust**2)
print(f"-> Simple combined: {combined_sig_simple:.2f}sigma")
print(f"-> Robust combined: {combined_sig_robust:.2f}sigma")
print("Note: Real combined significance would account for correlations")
print("-" * 95)
...
def run_fast_ensemble_with_loss_tracking(mc_reco, mc_gen, data_reco, n_runs=3, n_iters=2):
    ...
    for run in range(n_runs):
        print(f" > Run {run+1}/{n_runs}...", end=" ")
        run_losses = {'detector': [], 'generator': []}  # Store losses for this run
...
w_m_std_tracked, w_e_std_tracked, loss_history = run_fast_ensemble_with_loss_tracking(
    X_mc_reco, X_mc_gen, X_data_reco, n_runs=5, n_iters=4
)
...
plt.tight_layout()
loss_plot_path = os.path.join(OUTPUT_DIR, "Phase1_Training_Loss_Curves.png")
